...
export function SignUpForm({ onSuccess, onSwitchToSignIn }: SignUpFormProps) {
  ...
  useEffect(() => {
    authService.getUserCount().then(setUserCount)
  }, [])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const result = await authService.register(email, username, password, cleanPhone)

      if (result.success) {
        console.log("[v0] Registration successful - account created in Supabase")
        setSuccess(true)
        setTimeout(() => {
          onSuccess()
        }, 2000)
